...
export function HolidayImportDialog({
  ...
  const importMutation = useMutation({
    mutationFn: () =>
      api.post<{ created: number; skipped: number }>("/holidays/import", {
        rows: validRows.map((r) => r.data),
      }),
    onSuccess: (res) => {
      toast.success(`Imported ${res.created} holiday${res.created === 1 ? "" : "s"}.${res.skipped > 0 ? ` ${res.skipped} skipped.` : ""}`);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Bulk-add holidays from a CSV or Excel file. Download the template to see the expected columns.</DialogDescription>
          </DialogHeader>
                    ...
                    <TableBody>
                      {rows.map((r) => (
                        <TableRow key={r.rowNumber}>
                          ...
                          <TableCell>{r.raw[HOLIDAY_IMPORT_COLUMNS.name] || "—"}</TableCell>
                          <TableCell className="text-muted-foreground">
                            {r.data ? format(parseISO(r.data.date), "d MMM yyyy") : r.raw[HOLIDAY_IMPORT_COLUMNS.date] || "—"}
                          </TableCell>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            This will add <strong>{validRows.length}</strong> new holiday{validRows.length === 1 ? "" : "s"} to the calendar.
            {invalidRows.length > 0 && (
              <>
                {" "}
            ...
            This can&apos;t be undone in bulk — you&apos;d need to remove each one individually.
          </>
        }
        confirmLabel="Import"
